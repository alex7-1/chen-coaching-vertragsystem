...
def make_03(d):
    ...
    for i, page in enumerate(reader.pages):
        ...
        if i == 0:
            mak = d.get("tp_mak_nr", "")
            if mak:
                c.drawString(237.0, 666.1, mak)
            c.drawString(55.0, 598.5, f"{d['tp_vorname']} {d['tp_nachname']}")
            c.drawString(55.0, 569.6, d["tp_strasse"])
            c.drawString(306.0, 569.6, d["tp_plz_ort"])
            c.drawString(55.0, 540.6, f"{d['tp_vorname']} {d['tp_nachname']}")
            c.setFont("Helvetica", 9)
            c.drawString(307.0, 540.2, d["tp_iban"])
            c.setFont("Helvetica", 10)
            c.drawString(55.0, 511.7, d["tp_geldinstitut"])
            c.setFont("Helvetica", 9)
            c.drawString(307.0, 511.6, d["tp_bic"])
            c.drawString(438.0, 511.6, d["tp_steuernummer"])
            c.setFont("Helvetica", 10)
            c.drawString(174.0, 249.6, d["datum"])
            c.drawString(46.0,  170.1, d["tp_ort"])
            c.drawString(174.0, 170.1, d["datum"])
        elif i == 1:
            c.drawString(46.0,  734.6, f"{d['tp_vorname']} {d['tp_nachname']}")
            c.drawString(46.0,  704.4, d["tp_strasse"])
            c.drawString(301.3, 704.4, d["tp_plz_ort"])
            c.drawString(46.0,  196.0, d["tp_ort"])
            c.drawString(174.0, 196.0, d["datum"])
        c.save()
        packet.seek(0)
        overlay = PdfReader(packet)
        page.merge_page(overlay.pages[0])
